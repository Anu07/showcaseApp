...
const UploadVideoForm = ({ navigation }) => {
    ...
    const createFormData = (video, body = {}) => {
        ...
        data.append('filevideo', {
            name: video.filename,
            type: video.mime,
            uri: Platform.OS === 'ios' ? video.path.replace('file://', '') : photo.uri,
        });
    ...
    const notify = (message) => {
        if (Platform.OS != 'android') {
            ...
            ToastAndroid.show(message, ToastAndroid.LENGTH_LONG);
        }
    }
                ...
                <SafeAreaView style={styles.safeAreaView}>
                    <ScrollView style={{ height: '100%' }}>
                        <KeyboardAvoidingView
                            ...
                            <View style={[styles.customContainer, { backgroundColor: '#F8F7FD' }]}>
                                <Dropdown
                            ...
                            <Text style={{ margin: 10 }}>Choose video from:</Text>
                            ...
                            {video != null ? <Text style={{ marginLeft: 12, marginBottom: 20 }}>{video.filename} has been attached successfully.</Text> : <View />}
                            {loading == true ? <ActivityIndicator size='large' style={styles.loading} color="#F2B518" /> : <></>}
                            <Text style={{ margin: 10, color:'#929292' }}>Disclaimer: This video will be the sole property of Showcase Media, please refrain from racist or offensive language, no copyright infringement will be allowed.  The video will be reviewed and approved based on these factors.</Text>
